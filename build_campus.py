...
<div id="talk">
  <div class="talk-box">
    <div class="talk-avatar" id="talk-avatar">🧑‍🎓</div>
    <div class="talk-main">
...
(function(){var L=document.getElementById('legend'),C=window.CATS;
  Object.keys(C).forEach(function(k){var s=document.createElement('span');
    s.innerHTML='<i style="background:'+C[k].color+'"></i>'+C[k].label;L.appendChild(s);});})();
...
AFRAME.registerComponent('campus',{
  init:function(){
    ...
    for(var ni=0; ni<11; ni++){
      ...
      per.__talking=false; root.appendChild(per);
      window.NPCS.push({el:per, name:npcChat[ni][0], lines:npcChat[ni][1]});
    }
...
AFRAME.registerComponent('thirdperson',{
  ...
  tick:function(t,dt){
    ...
    if(moving){
      ...
      p.x=Math.max(-10.5,Math.min(10.5,p.x)); p.z=Math.max(-97,Math.min(13,p.z));
...
<a-scene pixel-world shadow="type: pcfsoft"
  ...
  <a-entity id="rig" position="0 0 9" thirdperson>
    <a-entity id="avatar">
      ...
      <a-box position="-0.12 2.07 -0.27" width="0.08" height="0.11" depth="0.02" material="shader: flat; color:#21262e"></a-box>
      <a-box position="0.12 2.07 -0.27" width="0.08" height="0.11" depth="0.02" material="shader: flat; color:#21262e"></a-box>
      <a-box position="0 1.9 -0.27" width="0.22" height="0.05" depth="0.02" material="shader: flat; color:#9a5b2a"></a-box>
    </a-entity>
