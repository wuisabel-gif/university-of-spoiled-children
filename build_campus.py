...
AFRAME.registerComponent('campus',{
  init:function(){
    ...
    // true if (x,z) is clear of existing colliders (buildings, trees, signs, props)
    function clearSpot(x,z,pad){ var C=window.COLLIDERS||[],i,o; pad=(pad==null?0.7:pad);
      for(i=0;i<C.length;i++){ o=C[i];
        if(o.t==='r'){ if(Math.abs(x-o.x)<o.hw+pad && Math.abs(z-o.z)<o.hd+pad) return false; }
        else { var dx=x-o.x, dz=z-o.z, rr=o.r+pad; if(dx*dx+dz*dz<rr*rr) return false; } }
      return true; }
    var BR='#b5482f';
    ...
    for(var bi=0; bi<26; bi++){
      var bx,bz,bok=false; for(var bt=0;bt<12&&!bok;bt++){ bx=(Math.random()<0.5?-1:1)*(4.5+Math.random()*4.5); bz=4-Math.random()*96; bok=clearSpot(bx,bz,0.7); }
      if(!bok) continue;
    ...
    for(var fb=0; fb<42; fb++){
      var fx,fz,fok=false; for(var ft=0;ft<12&&!fok;ft++){ fx=(Math.random()<0.5?-1:1)*(4.6+Math.random()*5.2); fz=4-Math.random()*96; fok=clearSpot(fx,fz,0.8); }
      if(!fok) continue;
    ...
    for(var li=0; li<32; li++){
      var lx2,lz2,lok=false; for(var lt=0;lt<12&&!lok;lt++){ lx2=(Math.random()<0.5?-1:1)*(4.8+Math.random()*9.6); lz2=4-Math.random()*122; lok=clearSpot(lx2,lz2,0.6); }
      if(!lok) continue;
